...
* Change log
** Version 1.8.1 <2023-11-01 Wed>
- Commenced Change Log.
- Added =minimum-config.el= to provide minimal configuration.
- Added ~citar-denote-nocite~ function to open Citar menu with all bibliographic entries neither cited nor referenced in any Denote file. This negates the need for ~citar-denote-reference-nocite~ because notes can be created from within the Citar menu.
- Existing reference removed from Citar selections when adding a new reference to a note.
- Added ~citar-denote-signature~ variable to allow adding signatures to new notes.
- Fixed bug in ~citar-denote-add-citekey~ and ~citar-denote-remove-citekey~ when using multiple file types.
