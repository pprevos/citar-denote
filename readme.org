...
=======
The =citar-denote-cite-citation= and =citar-denote-reference-nocite= functions find bibliographic entries in the Citar menu not cited or referenced in your corpus of Denote notes.

The first function lets you select unused entries and cite them in a Denote file. The second function create a new bibliographic note for the first selected entry.
>>>>>>> Stashed changes
