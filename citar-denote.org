...
When only one note cites the selected entry, then this file is opened. When more than one file cites the chosen entry, then you first need to select which file to jump to.

This function only looks at citations in your document by default. Setting the ~citar-denote-cite-includes-reference~ to non-nil includes references.
